...
class CrimeService:
    ...
    async def register_crime(self, data: CrimeCreate, registered_by: User) -> Crime:
        ...
        # Note: pass the CrimeMO object directly (already in session memory
        # after flush above). No second DB round-trip needed.
        mo_obj = mo if mo_source else None
        dna_svc = DNAService(self.db)
        await dna_svc.create_pending(created, mo_obj)

        # Eagerly load/refresh mo_features relation to avoid MissingGreenlet during serialization
        await self.db.refresh(created, attribute_names=["mo_features"])
    ...
    async def get_crime_by_fir(self, fir_number: str) -> Crime:
        ...
        # Eagerly load/refresh mo_features relation to avoid MissingGreenlet during serialization
        await self.db.refresh(crime, attribute_names=["mo_features"])
        return crime
    ...
    async def update_crime(
        ...
        # Eagerly load/refresh mo_features relation to avoid MissingGreenlet during serialization
        await self.db.refresh(updated, attribute_names=["mo_features"])
